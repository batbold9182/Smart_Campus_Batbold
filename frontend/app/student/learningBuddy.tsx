  ...
  if (loading) {
    return (
      <SafeAreaView className="flex-1 items-center justify-center bg-app-bg" edges={["top"]}>
        <AppButton title="Loading Learning Buddy..." loading={true} className="bg-transparent" textClassName="mt-3 text-app-base text-app-muted" onPress={() => {}} />
      </SafeAreaView>
    );
  }
        ...
        <View className="border-b border-app-border-light bg-app-surface px-5 pb-4 pt-2">
          <View className="flex-row items-center justify-between">
            <View className="flex-1 pr-3">
              <Text className="text-app-xl font-bold text-app-text">📘 Learning Buddy</Text>
          ...
          <View className="mt-4 flex-row flex-wrap gap-2">
            ...
            <View className="rounded-full bg-app-success-bg px-3 py-2">
              <Text className="text-app-xs font-semibold text-app-success">{onlineCount} students online</Text>
            </View>
            {user?.program ? (
        ...
        >
          ...
          {messages.map((message) => {
            ...
            return (
              ...
              >
                {!isMine ? (
                  <Text className="mb-1 text-app-xs font-semibold text-app-success-accent">
                    {message.sender.name}
                ...
                <Text className={`mt-2 text-app-xs ${isMine ? "text-app-success-timestamp" : "text-app-placeholder"}`}>
