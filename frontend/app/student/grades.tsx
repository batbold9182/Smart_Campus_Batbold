    ...
    <ScreenLayout title="Grades" backRoute="/student/dashboard">
        ...
        <View className="mb-4 flex-row justify-between gap-2">
          <View className="flex-1 rounded-xl bg-app-surface p-4 shadow-card">
            <Text className="text-app-lg font-bold text-app-text">{data?.summary.courseCount || 0}</Text>
            <Text className="mt-1 text-app-xs text-app-muted">Courses</Text>
          </View>
          <View className="flex-1 rounded-xl bg-app-surface p-4 shadow-card">
            <Text className="text-app-lg font-bold text-app-text">{data?.summary.gradedCount || 0}</Text>
            <Text className="mt-1 text-app-xs text-app-muted">Published Grades</Text>
          </View>
          <View className="flex-1 rounded-xl bg-app-surface p-4 shadow-card">
            <Text className="text-app-lg font-bold text-app-text">{data?.summary.averageGrade ?? "-"}</Text>
            <Text className="mt-1 text-app-xs text-app-muted">Average</Text>
          </View>
        </View>

        {!data?.items.length ? (
          <View className="mb-4 rounded-xl bg-app-surface p-4 shadow-card">
            <Text className="mb-2 text-app-base font-semibold text-app-text">No Grades Yet</Text>
